...
@if ($btype == 'outline')
    <{{$tag}}
        {{ $attributes->merge(['type' => 'button', 'class' => "btn text-nowrap border fw-bold btn-light text-{$variantBg} btn-{$size}"]) }}>
        @isset($icon)
...
@elseif($btype == 'solid')
    <{{$tag}}
        {{ $attributes->merge(['type' => 'button', 'class' => "btn text-nowrap text-{$variantTxt} btn-{$size} btn-{$variantBg}"]) }}>
        @isset($icon)
            <span class="material-icons md-{{ $iconSize }} align-middle"> {{ $icon }} </span>
        @endisset {{$slot}}
    </{{$tag}}>
