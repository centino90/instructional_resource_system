                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            <x-real.card :vertical="'center'">
                                ...
                                <x-slot name="action">
                                    <x-real.btn data-bs-toggle="modal" data-bs-target="#courseLessonModal">+ New
                                        Standard
                                    </x-real.btn>
                                </x-slot>
                                <x-slot name="body">
                                    <ul class="list-group" id="verbGroup">
                                        <x-real.form method="put"
                                            action="{{ route('admin.typology.update', $typology) }}"
                                                ...
                                                <li class="list-group-item position-relative">
                                                    ...
                                                    <h5>
                                                        <input type="text" name="key-{{ $key }}"
                                                            value="{{ $key }}" class="text-uppercase border-0 focusable"
                                                            readonly>
                                                    </h5>
                                                        ...
                                                        @foreach ($verbs as $verb)
                                                            <span class="text-nowrap">
                                                                <input type="text" name="property-{{ $key }}[]"
                                                                    value="{{ $verb }}"
                                                                    class="text-uppercase border-0 focusable" readonly>
                                                                <small
                                                    ...
                                                    <div class="col-12">
                                                        <x-real.btn class="addVerb"
                                                            data-key="{{ $key }}" :size="'sm'">+ Add verb
                                                        </x-real.btn>
                                                    </div>
                                                </li>
                                            @endforeach
                                            <x-slot name="submit">
                                                <div class="hstack justify-content-between my-3">
                                                    <x-real.btn :size="'lg'" :tag="'a'" href="{{route('admin.typology.reset')}}" onclick="return confirm('Are you sure you want to reset changes?')">
                                                        Reset
                                                    </x-real.btn>
                                                    <x-real.btn :size="'lg'" type="submit" class="no-loading">
                                                        Submit Changes
                                                    </x-real.btn>
                                                </div>
                                            </x-slot>
                ...
                <div class="modal-body px-5 py-5">
                    <div class="row">
                        <x-real.form id="storeTypologyForm">
                            <div class="col-12">
                                <x-real.alert class="d-none mb-4" :variant="'danger'" :dismiss=false>Checked Verb
                                    should not be empty
                                </x-real.alert>
                            </div>
                            ...
                            <x-real.btn class="createMore">+ Add verb recommendation</x-real.btn>
    ...
    @section('script')
        <script>
            $(document).ready(function() {
                ...
                $('#storeTypologyForm').delegate('.verbRemove', 'click', function(event) {
                    $(event.target).closest('.position-relative').remove();
                ...
                $('.createMore').click(function(event) {
                    ...
                    $parent.append(`
                    <div class="position-relative recommendedVerbs">
                        <x-real.input name="recommended_verb[]">
                            <x-slot name="label">Recommended verb</x-slot>
                        </x-real.input>
                        <h5 class="verbRemove fw-bold btn btn-sm text-danger position-absolute end-0 top-0">
                ...
                $('#storeTypologyForm').on('submit', function(event) {
                    ...
                    const $recommendedVerbs = $(event.target).find('[name="recommended_verb[]"]').map(
                function() {
                        return $(this).val()
                    })
                    ...
                    if (!$checkedVerb) {
                        $('#storeTypologyForm .alert').removeClass('d-none')
                        return;
                    }
                    console.log($checkedVerb)
                    $('#storeTypologyForm .alert').addClass('d-none')
                ...
                `)
                    ...
                    $('#storeTypologyForm .recommendedVerbs').remove();
                    $('#storeTypologyForm')[0].reset()
