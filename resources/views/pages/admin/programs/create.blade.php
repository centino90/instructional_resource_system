                           ...
                           <x-real.form action="{{ route('admin.programs.store') }}">
                              ...
                              <x-real.input name="is_general" type="select">
                                <x-slot name="label">Add as General Program?</x-slot>
                                 <x-slot name="options">
                                    <option {{old('is_general') == false ? 'selected' : ''}} value="0">No</option>
                                    <option {{old('is_general') == true ? 'selected' : ''}} value="1">Yes</option>
                                 </x-slot>
                              </x-real.input>
                                 ...
                                 <x-slot name="rows">
                                    @foreach ($deans as $user)
                                       <tr>
                                          ...
                                          <td>
                                             @if (!empty(old('program_dean')) && isset(old('program_dean')[$user->id]) && boolval(old('program_dean')[$user->id]))
                                                <x-real.input type="radio" name="program_dean[{{ $user->id }}]" value="1"
                                                   checked>
                                                </x-real.input>
                                             @else
                                                <x-real.input type="radio" name="program_dean[{{ $user->id }}]"
                                                   value="1">
                                                </x-real.input>
                                             @endif
                                          </td>
