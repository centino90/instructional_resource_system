<x-app-layout>
    ...
    <div class="row g-3">
        ...
        <div class="col-4">
            <x-real.card>
                <x-slot name="header">Updated Properties</x-slot>
                <x-slot name="body">
                    <ul class="nav flex-column gap-2">
                        @forelse ($activity->properties as $key => $property)
                            @if (!is_array($property))
                                <li class="nav-item border-bottom">
                                    <x-real.text-with-subtitle>
                                        ...
                                        <x-slot name="subtitle">{{ $key ?? 'No label' }}</x-slot>
                                    </x-real.text-with-subtitle>
                                </li>
                            @endif
                        @empty
                            <x-real.no-rows>
                                <x-slot name="label">There are no updated properties</x-slot>
                            </x-real.no-rows>
                        @endforelse
                    </ul>
                </x-slot>
